...
add the points for a tetrahedron
====================*/
void add_tetrahedron( struct matrix * polygons,
double x1, double y1, double z1,
double x2, double y2, double z2,
...
double x4, double y4, double z4) {
	...
	add_polygon(polygons, x1, y1, z1, x4, y4, z4, x2, y2, z2);

	add_polygon(polygons, x1, y1, z1, x3, y3, z3, x4, y4, z4);
	add_polygon(polygons, x2, y2, z2, x4, y4, z4, x3, y3, z3);

}//end add_tetrahedron

void add_triprism ( struct matrix * polygons, 
double x0, double y0, double z0,
double x1, double y1, double z1,
...
of a cylinder with base center (cx, cy, cz) and
...
adds all the points for an ellipsoid with center
(cx, cy, cz) and radius r.

should call generate_ellipsoid to create the
necessary points
====================*/
void add_ellipsoid( struct matrix * edges,
double cx, double cy, double cz,
double a, double b, double c, int step ) {

	struct matrix *points = generate_ellipsoid(cx, cy, cz, a, b, c, step);
...
of an ellipsoid with center (cx, cy, cz) and
radius r.
Returns a matrix of those points
====================*/
struct matrix * generate_ellipsoid(double cx, double cy, double cz,
double a, double b, double c, int step ) {
